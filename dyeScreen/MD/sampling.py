...
import commons.geom_utils as gu
...
class scan_cofigs():
    ...
    def move_double_att(self, a, b):
        ...
        pairs = self.bond_labels[a-1]+'&'+self.bond_labels[a]+'-'+self.bond_labels[b]+'&'+self.bond_labels[b+1]
    ...
    def check_valid_sample(self, a, b, dist_min):
        ''' Checks taht a-b pair is within dist_min, or is not a terminal residue'''
...
def scan_DNAconfig(pdb_monomer, pdb_dna, path_save, resD, resL=None, 
                   ...
                   box_type='doubleAtt', max_pairs=80):
    ...
    for (i, j) in valid_pairs:
        ...
        elif attachment=='double': #!!! Works but not deleting the correct nuc at the end
            # Double attachment
            ### i is 2nd attachment of 1st molecules, j is 1st atachement of 2nd molecule
        ...
        print(f"{idx}: ({i},{j}), {configs.pairs[idx]}, {len(configs.pairs[idx])}")
        idx += 1
...
def gen_leap_nolinker(path_samples, amber_path, dye_attach, dna_attach, 
    ...
    Args:
        path_samples (str): Path to save sample files
        amber_path (str): path with amber executables
        dye_attach (list): List with names of the dye atoms that form DNA attachement
        dna_attach (list): List with names of the DNA atoms that form DNA attachement
        dye_mol2 (str): mol2 file path for dye
        frcmod (str: frcmod file path
        dye_name (str, optional): resname for the dye. Defaults to 'DYE'.
        ions (list, optional): list of ions to add 
            [<ion AMBER name>, <charge>]. Defaults to ['NA', '0'].
        wbox (float, optional): Size of water box in ams. Defaults to 20.
    """
...
def modify_mol2(opo3h, mol2_file, mol2_out=None, attachment='double', dye_file=None, parmchk2=None):
    ...
    Args:
        opo3h (list): List of atoms names for OPO3H group (in that order).
        mol2_file (str): Name of existing mol2 file of dye
        mol2_out (str, optional): Name of output mol2 file. Defaults to None.
        attachment (str, optional): Type of linker attachement: Single or Double. 
            Defaults to 'double'.
        dye_file (str, optional): _description_. Defaults to None.
        parmchk2 (str, optional): _description_. Defaults to None.

    Raises:
        NotImplementedError: _description_
    """

    import FF.file_process as fp
    from FF.gen_dye_lnk_files import gen_frcmod
