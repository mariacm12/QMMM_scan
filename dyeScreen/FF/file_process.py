...
def calculate_charges(df, charge_col='charge'):
    ''' Only for mol2
    Calculate total for the column charge
    '''
    charge = df[charge_col].sum()
    return charge

def del_atoms(df, del_ids, ids_col='atom_id', printing='atom_name', pdb=False, del_later=None):
    '''
    ...
    for idx in range(len(del_ids)):
        ...
        #deleted_ats.append([deleted_row[ids_col].item()] + [deleted_row[printing].item()])
        ...
        if not pdb:
            ...
            #df_at.loc[df_at] = np.where(df_at[ids_col] < ids, df_at[ids_col], df_at[ids_col]-1)
            del_ids -= 1 # also for the list of atoms to delete
            df_bonds, deleted_bnd = delete_bonds(df_bonds, ids, deleted_bnd)
    #df_at[ids_col] = np.arange(1,len(df_at)+1)
...
class FRCMOD_DF():
    ...
    def __init__(self) -> None:
        ...
        # Initializing frcmod dictionary
        self.data = {}
        self.data['MASS'] = pd.DataFrame()
        self.data['BOND'] = pd.DataFrame()
        self.data['ANGLE'] = pd.DataFrame()
        self.data['DIHEDRAL'] = pd.DataFrame()
        self.data['IMPROPER'] = pd.DataFrame()
        self.data['NONBOND'] = "" # Input format depends on the molecule so has to be entered manually
        self.title = ""

        # Definition: (PK/IDIVF) * (1 + cos(PN*phi - PHASE))

        # Defining keys
        self.mass_keys = ['symbol', 'atmass', 'atpol', 'text']
        self.bond_keys = ['symbol', 'force', 'eq', 'text']
        self.angl_keys = ['symbol', 'force', 'eq', 'text']
        self.dihe_keys = ['symbol', 'idivf', 'pk', 'phase', 'pn', 'text']
        self.impr_keys = ['symbol', 'pk', 'phase', 'pn', 'text']
        ...
        def read_file(self, pdb_file):
            # Read and return warning if values are missimg from force field: make if for EACH section
            if False: # True when section contains "ATTN, need revision"
                print("the symbol that needs additional info")
                # Could also return error. Also print format needed for the input
            return 
...
def get_res_from_coord(PDB, coord):
    """Given a coordinate, search in the PDB file the residue it corresponds to
